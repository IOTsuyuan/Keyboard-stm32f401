...
/* USER CODE BEGIN PV */
/* Private variables ---------------------------------------------------------*/
extern unsigned char USB_Recive_Buffer[5];
...
static int8_t CUSTOM_HID_OutEvent_FS(uint8_t event_idx, uint8_t state)
{
  ...
  */
  ...
  USBD_CUSTOM_HID_HandleTypeDef   *hhid; //定义�?个指向USBD_CUSTOM_HID_HandleTypeDef结构体的指针
  ...
  {
    /*
    收到的数据中第一位是设备号，第二位是键盘几个灯的状�?�信�? 
    Caps  NumL  ScrL
    对应三位二进制数，最左侧是最低位
    例如当Caps是亮的其他都是灭的，收到的数据是0
    */
    USB_Recive_Buffer[i]=hhid->Report_buf[i];  //把接收到的数据�?�到自定义的缓存区保存（Report_buf[i]为USB的接收缓存区�?
      // printf("USB_Recive_Buffer[%d] = 0x%x \r\n",i,USB_Recive_Buffer[i]); //打印接收到的信息，确认是否正确，调试�?
  } 
  return (USBD_OK);
  /* USER CODE END 6 */
}
